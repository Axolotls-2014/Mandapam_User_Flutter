...
class SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> _checkAppVersion() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['version'] != null && (data['version'] as List).isNotEmpty) {
          for (var version in data['version']) {
            if (version['key'] == 'user_app_version') {
              setState(() {
                userAppVersion = version['value'];
      ...
      print("Error fetching app version2: $e");
    }
  }
  ...
  Future<void> _launchUpdateUrl() async {
    const url = 'https://play.google.com/store/apps/details?id=com.mandapam.user&pcampaignid=web_share';
    if (await canLaunchUrl(Uri.parse(url))) {
      await launchUrl(Uri.parse(url));
    } else {
      Get.snackbar('Error', 'Could not launch update URL');
