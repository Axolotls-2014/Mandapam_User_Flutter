...
class CategoryController extends GetxController implements GetxService {
  ...
  Future<void> getCategoryList(bool reload, {bool allCategory = false}) async {
    if (_categoryList == null || reload) {
      ...
      if (categoryList != null) {
        ...
        // Initialize selected list for each category
        for (int i = 0; i < _categoryList!.length; i++) {
          _interestSelectedList!.add(false);
        }
      }

      // Trigger update after successfully fetching and updating the list
      update();
    }
  }
  ...
  void getSubCategoryList(String? categoryID) async {
    ...
    if (subCategoryList != null) {
      _subCategoryList= [];
      _subCategoryList!.add(CategoryModel(id: int.parse(categoryID!), title: 'all'.tr));
